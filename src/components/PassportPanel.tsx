...
export function PassportPanel({ passport }: { passport: Passport }) {
  ...
  return <div className="passport-stack">
    <section className="passport-overview">
      <div className="passport-verdict"><span className="eyebrow">Solana stock passport</span><div className={`passport-state state-${passport.state.toLowerCase()}`}>{passport.state}</div><p>{passport.state === 'PASS' ? 'Required issuer and onchain checks agree at this evidence boundary.' : passport.state === 'CAUTION' ? 'The asset is identifiable, but one visible source warning needs attention.' : passport.state === 'BLOCKED' ? 'A required integrity check failed. Downstream use must stop at this evidence boundary.' : 'A required evidence layer is missing. No clean passport was issued.'}</p></div>
      <div className="primary-facts"><div><span>Confirmed slot</span><strong>{passport.rpcSlot == null ? 'Unavailable' : passport.rpcSlot.toLocaleString('en-US')}</strong><em>Solana mainnet RPC</em></div><div><span>Reported reserve coverage</span><strong>{passport.reserveCoverage == null ? 'Unavailable' : `${(passport.reserveCoverage * 100).toFixed(3)}%`}</strong><em>Shares held ÷ circulating</em></div><div><span>Effective multiplier</span><strong>{passport.multiplier == null ? 'Unavailable' : passport.multiplier.toFixed(9)}</strong><em>Token-2022 Scaled UI</em></div></div>
