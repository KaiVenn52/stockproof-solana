...
export function ReplayLab({ passport }: { passport: Passport }) {
  ...
  return <main className="content-view replay-view">
    ...
    <section className="panel benchmark-summary"><div><strong>{instruments.length}</strong><span>pinned Solana xStocks</span></div><div><strong>6</strong><span>deterministic checks</span></div><div><strong>2</strong><span>source classes</span></div><div><strong>1.0</strong><span>versioned JSON schema</span></div></section>
    ...
    <section className="panel replay-table"><table><caption className="sr-only">Trust boundaries exposed by a StockProof passport, the inputs each one surfaces, and how a failure is handled</caption><thead><tr><th scope="col">Boundary</th><th scope="col">Inputs exposed</th><th scope="col">Failure behavior</th></tr></thead><tbody>{surfaces.map(([name, inputs]) => <tr key={name}><td>{name}</td><td>{inputs}</td><td>Warn, block, or abstain</td></tr>)}</tbody></table><p className="table-note">The current prototype uses xStocks public endpoints and Solana mainnet RPC. It is intentionally read-only: the artifact is the evidence-backed passport, not a trade.</p></section>
