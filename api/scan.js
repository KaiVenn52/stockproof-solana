...
export default async function handler(req, res) {
  ...
  try {
    ...
    const reserveCheckState = reserveState(coverage)
    ...
    const timeline = [
      { id: 't1', time: time(reserves.timestamp), title: 'Reserve proof', detail: coverage == null ? 'Coverage unavailable' : `${(coverage * 100).toFixed(2)}% reported coverage`, kind: 'underlying', offset: 12 },
    ...
    const brief = `${symbol} resolves to ${identityState === 'pass' ? 'the expected' : 'an unexpected'} Solana Token-2022 mint at confirmed slot ${account.context.slot}. The effective onchain balance multiplier ${effectiveMultiplier?.toFixed(9) ?? 'could not be read'} ${multiplierCheckState === 'pass' ? 'matches' : 'does not cleanly match'} issuer metadata, and reported reserve coverage is ${coverage == null ? 'unavailable' : `${(coverage * 100).toFixed(3)}%`}. The mint pause flag is ${paused === false ? 'off' : paused === true ? 'on' : 'unavailable'}.${warning}`
