...
const stateForCoverage = (coverage) => coverage >= 1 ? 'pass' : coverage >= 0.995 ? 'caution' : 'fail'
const stateForMultiplier = (issuer, chain) => {
  const delta = Math.abs(issuer - chain)
  return delta <= 1e-9 ? 'pass' : delta <= 1e-6 ? 'caution' : 'fail'
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [asset, price, multiplier, reserves, corporate] = await Promise.all([
      json(`${XSTOCKS_BASE}/public/assets/${encoded}`, controller.signal),
      json(`${XSTOCKS_BASE}/public/assets/${encoded}/price-data`, controller.signal),
    ...
    const [account, supply] = await Promise.all([
      ...
      rpc('getTokenSupply', [mint, { commitment: 'confirmed' }], controller.signal),
    ])
    ...
    const effectiveMultiplier = scaled
      ? number(nowSeconds >= Number(scaled.newMultiplierEffectiveTimestamp) ? scaled.newMultiplier : scaled.multiplier)
      : null
    const issuerMultiplier = number(multiplier.currentMultiplier)
    const coverage = number(reserves.sharesHeld) / number(reserves.circulatingSupply)
    const identityState = mint === meta.mint && asset.symbol === symbol ? 'pass' : 'fail'
    const programState = account.value.owner === TOKEN_2022_PROGRAM && account.value.data.program === 'spl-token-2022' ? 'pass' : 'fail'
    const multiplierCheckState = effectiveMultiplier == null || !Number.isFinite(issuerMultiplier) ? 'unknown' : stateForMultiplier(issuerMultiplier, effectiveMultiplier)
    const reserveCheckState = Number.isFinite(coverage) ? stateForCoverage(coverage) : 'unknown'
    ...
    const checks = [
      ...
      { id: 'multiplier', title: 'Corporate-action multiplier', summary: 'Issuer multiplier agrees with the effective Scaled UI value onchain', state: multiplierCheckState, result: effectiveMultiplier == null ? 'UNAVAILABLE' : effectiveMultiplier.toFixed(9), detail: 'Solana raw balances require the Token-2022 Scaled UI multiplier. StockProof selects the effective value by timestamp before comparing it with issuer metadata.', observations: [{ label: 'Issuer API', value: Number.isFinite(issuerMultiplier) ? issuerMultiplier.toFixed(12) : 'Unavailable', accent: multiplierCheckState }, { label: 'Effective onchain', value: effectiveMultiplier == null ? 'Unavailable' : effectiveMultiplier.toFixed(12), accent: multiplierCheckState }] },
      { id: 'reserves', title: 'Reserve coverage', summary: 'Reported custody shares cover reported circulating tokens', state: reserveCheckState, result: Number.isFinite(coverage) ? `${(coverage * 100).toFixed(3)}%` : 'UNAVAILABLE', detail: 'The comparison uses issuer-reported proof-of-reserves fields without rounding the ratio into compliance. Total mint supply is shown separately because inventory and system wallets can make it differ from circulating supply.', observations: [{ label: 'Shares held', value: String(reserves.sharesHeld) }, { label: 'Circulating', value: String(reserves.circulatingSupply) }, { label: 'PoR timestamp', value: reserves.timestamp }] },
    ...
    const evidence = [
      ...
      { id: 'chain', title: 'Solana mint account', summary: `${account.value.data.program} at confirmed slot ${account.context.slot}.`, state: programState, timestamp: time(scannedAt), source: 'Solana mainnet RPC', endpoint: 'getAccountInfo + getTokenSupply', retrievedAt: scannedAt, href: explorer },
      { id: 'reserves', title: 'Proof of reserves', summary: `${reserves.sharesHeld} shares reported against ${reserves.circulatingSupply} circulating tokens.`, state: reserveCheckState, timestamp: time(reserves.timestamp), source: 'xStocks proof of reserves', endpoint: `/public/proof-of-reserves/${symbol}`, retrievedAt: scannedAt },
      { id: 'multiplier', title: 'Balance multiplier', summary: `Issuer ${issuerMultiplier.toFixed(9)} · effective onchain ${effectiveMultiplier?.toFixed(9) ?? 'unavailable'}.`, state: multiplierCheckState, timestamp: time(scannedAt), source: 'xStocks API + Solana Token-2022', endpoint: `/public/assets/${symbol}/multiplier + getAccountInfo`, retrievedAt: scannedAt },
    ...
    const timeline = [
      { id: 't1', time: time(reserves.timestamp), title: 'Reserve proof', detail: `${(coverage * 100).toFixed(2)}% reported coverage`, kind: 'underlying', offset: 12 },
    ...
    const brief = `${symbol} resolves to the expected Solana Token-2022 mint at confirmed slot ${account.context.slot}. The effective onchain balance multiplier ${effectiveMultiplier?.toFixed(9) ?? 'could not be read'} ${multiplierCheckState === 'pass' ? 'matches' : 'does not cleanly match'} issuer metadata, and reported reserve coverage is ${(coverage * 100).toFixed(3)}%. The mint pause flag is ${paused === false ? 'off' : paused === true ? 'on' : 'unavailable'}.${warning}`
    ...
    return res.status(200).json({
      ...
      displayedSupply: supply.value?.uiAmount ?? null,
      ...
      timeline,
      brief,
      researchAction: state === 'PASS' ? 'The required identity, chain, multiplier, and reserve checks agree. Downstream apps can consume this passport with its evidence timestamps.' : 'Inspect the warning before a downstream app treats this asset state as clean. Re-scan after the affected source updates.',
